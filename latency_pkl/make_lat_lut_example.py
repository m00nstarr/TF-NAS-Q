...
from tools.utils import measure_latency_in_ms, measure_act_memory
...
def get_peak_memory_lookup(is_cuda):
	peak_memory_lookup = OrderedDict()
	...
	# # time.sleep(0.1)
	# peak_memory_lookup['base'] = lat1 + lat2 + lat3 + lat4 + lat5 # + 0.1  # 0.1 is the latency rectifier
	...
	for idx in range(len(PRIMITIVES)):
		...
		for mc in mc_list:
			...
			print(block.mid_channels)
			peak_memory_lookup[key][block.mid_channels] = mem
			# time.sleep(0.1)

	print(peak_memory_lookup)
	...
	for idx in range(len(PRIMITIVES)):
		...
		for mc in mc_list:
			...
			shape = (1, 192, 7, 7) if is_cuda else (1, 192, 7, 7)
			mem   = measure_act_memory(block, shape, is_cuda)
			if idx < 4:
...
if __name__ == '__main__':
	...
	peak_memory_lookup = get_peak_memory_lookup(is_cuda=False)
