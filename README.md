...
1. TF-NAS 와 TF-NAS-Q의 차이점


2. 실험 결과에 대한 추가적인 분석  
- loss 에 대한 분석
- acc. 가 기존 방식 보다 높게 나온 점.
- 양자화를 했음에도 정확도가 더 높은 이유  


3. 모델 경량화 연구가 활발히 이루어지고 있는데, 제안한 방식이 갖는 차별점이 무엇인지.  
- TF-NAS-Q의 경우, 소형 마이크로 컨트롤러의 매우 작은 제한된 메모리 내에서 model inference가 가능하도록 인공지능신경망을 탐색하는데 차별점이 있다. 
- 
